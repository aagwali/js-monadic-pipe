...
export const tryValidateEnvKeys = (mandatoryKeys: string[], env: any
): Validation<string[], IValidationAcc> =>
    mandatoryKeys.map(tryValidate(env)(R.prop))
        .reduce((acc, validation) => validation.ap(acc.acc()))

export const tryBuildConfig = (env: any
): Validation<AppError, Config> =>
